...
# 1. Replace EmergencySOSScreenAlt
sos_regex = re.compile(r'export\s+const\s+EmergencySOSScreenAlt:\s*React\.FC[\s\S]*?(?=export\s+const\s+LocalServicesScreen)')

sos_replacement = """export const EmergencySOSScreenAlt: React.FC = () => {
...
# 2. Replace LocalServicesScreen with ExploreScreen
explore_regex = re.compile(r'export\s+const\s+LocalServicesScreen:\s*React\.FC[\s\S]*?(?=export\s+const\s+RentalsPageScreen)')

explore_replacement = """export const LocalServicesScreen: React.FC = () => {
  const navigate = useNavigate();
  const { theme } = useApp();
...
# 3. Replace JobsDetailScreen
job_regex = re.compile(r'export\s+const\s+JobsDetailScreen:\s*React\.FC[\s\S]*?(?=export\s+const\s+DrawerScreen)')

job_replacement = """export const JobsDetailScreen: React.FC = () => {
  const navigate = useNavigate();
  const { theme } = useApp();
...
# 4. Append new screens at the SettingsScreen match point
end_regex = re.compile(r'// ==========================================\s*\r?\n\/\/ 31\. SETTINGS SCREEN')
append_content = """// ==========================================
...
assert sos_regex.search(content) is not None, "sos_regex not matched"
assert explore_regex.search(content) is not None, "explore_regex not matched"
assert job_regex.search(content) is not None, "job_regex not matched"
assert end_regex.search(content) is not None, "end_regex not matched"

# Use lambdas to prevent escaping issues
content = sos_regex.sub(lambda m: sos_replacement, content)
content = explore_regex.sub(lambda m: explore_replacement, content)
content = job_regex.sub(lambda m: job_replacement, content)
content = end_regex.sub(lambda m: append_content, content)
